    ...
    handlers: {
      GET: async () => {
        ...
        const [{ data: categories }, { data: products }] = await Promise.all([
          ...
          supabaseAdmin.from('products').select('id,name,description,price,duration_days,stock,image_url,category_id,is_active').eq('is_active', true).order('created_at', { ascending: false }),
        ]);
